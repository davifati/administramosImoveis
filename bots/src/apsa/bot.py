...
from login_page import ApsaLoginPage
from home_page import ApsaHomePage
from download_page import ApsaDownloadPage
        ...
        try:
            ...
            if self.home_page.check_pop_up():
                if self.home_page.check_login_success():
                    ...
                    self.add_report(
                        reports,
                        f"Login falhou após pop-up para o usuário: {username}",
                        "FAIL",
                    )
                    return
                ...
                if isinstance(boletos_info, list):
                    for boleto in boletos_info:
                        if boleto.get("download_concluido", False):
                            ...
                            try:
                                ...
                                if link_pdf_boleto:
                                    ...
                                    try:
                                        ...
                                        self.add_report(
                                            reports,
                                            f"Boleto salvo no MySQL para o usuário: {username}",
                                            "OK",
                                        )
                                    except Exception as e:
                                        self.add_report(
                                            reports,
                                            f"Erro ao salvar boleto no MySQL para o usuário: {username}: {e}",
                                            "FAIL",
                                        )
                                else:
                                    self.add_report(
                                        reports,
                                        f"Falha ao enviar o boleto para o S3 para o usuário: {username}",
                                        "FAIL",
                                    )
                            except FileNotFoundError as e:
                                self.add_report(
                                    reports,
                                    f"Erro ao localizar o boleto PDF na pasta: {e}",
                                    "FAIL",
                                )
                            ...
                            self.add_report(
                                reports,
                                f"Falha ao fazer download do boleto para o usuário: {username}",
                                "FAIL",
                            )
                else:
                    self.add_report(
                        reports,
                        "Nenhum boleto encontrado ou erro ao obter boletos.",
                        "FAIL",
                    )
...
if __name__ == "__main__":
    query = DynamoDBQuery()
    items = query.getAdminLoginDetails(
        administradora="apsa (login: 32179787 senha 123456)"
    )
