...
from login_page import VortexLoginPage
from home_page import VortexHomePage
from download_page import VortexDownloadPage
    ...
    def run(self, username, password, endereco):
        ...
        try:
            ...
            if self.home_page.check_login_success():
                ...
                self.add_report(
                    reports,
                    f"Erro ao tentar checar hom page para usuário: {username}",
                    "FAIL",
                )
                return
            ...
            if boletos_disponiveis:
                boleto_info = self.download_page.get_boleto_info(
                    self.download_dir, boletos_disponiveis, endereco
                )
                self.add_report(
                    reports,
